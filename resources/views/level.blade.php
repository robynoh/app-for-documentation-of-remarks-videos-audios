...
<div class="row-fluid">
                        ...
                        <div class="block">
                            <div class="navbar navbar-inner block-header">
                                <div class="muted pull-left"><b style="font-size:14px; color:#000"> Level(s)</b></div>
                            </div>
...
@endif
								...
								<form action="add_level"  class="form-horizontal" enctype="multipart/form-data" method="POST">
						
                            	{{ csrf_field() }}
  							
  								
  								
  									<input type="text" name="level_name" data-required="1" class="span6 m-wrap" >
								<button name="add_school" class="btn btn-primary" ><i class="icon-plus icon-white"></i> Add Level</button>
                                    ...
                                    <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example2" style="font-size:12px">
                                        ...
                                        @foreach($levels as $level)
                                            <tr class="gradeU">
                                                ...
                                                <td><b>{{$level->level_id}}</b></td>
                                                <td>{{$level->level}}</td>
												
											   
											    <td class="center"><div class="btn-group">
                                                    <a  id="deleteUser{{ $level->level_id}}" data-userid="{{ $level->level_id}}" href="javascript:void(0)" onclick="showAlert({{ $level->level_id}});" ><button class="btn btn-danger"><i class="icon-remove icon-white"></i></button></a></div></td>
                                            	@endforeach
                                            </tr>
                                        </tbody>
                                    </table>
...
function senddel(){
    ...
    window.location="/level/delete/"+$('#app_id').val();
